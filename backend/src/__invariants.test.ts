 ...
 *
 * Run: bun test backend/src/__invariants.test.ts
 */
...
describe('INV-7: hooks/ directory has been removed', () => {
  it('backend/src/hooks/ directory does not exist', () => {
...
describe('INV-9: All org-MCP tools have audit wrapping', () => {
  it('MCP tools are wrapped with audit at the assembly boundary', () => {
    // MCP tools are audited in two places:
    // 1. message-handler.ts wraps MCP tools with withAudit when assembling sessions
    // 2. http-server.ts logs McpToolCall events for all org-MCP tool invocations
    ...
    // http-server.ts must have audit logging for org-MCP tool calls
    const httpServerPath = join(SRC_ROOT, 'org-mcp', 'http-server.ts');
    expect(existsSync(httpServerPath), 'org-mcp/http-server.ts must exist').toBe(true);
    const hsContent = readFileSync(httpServerPath, 'utf-8');
    const hasAuditLog = /McpToolCall/.test(hsContent);
    expect(
      hasAuditLog,
      'org-mcp/http-server.ts does not have McpToolCall audit logging',
    ).toBe(true);
  });
});
