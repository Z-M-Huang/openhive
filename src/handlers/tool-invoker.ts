/**
 * Org tool invoker — direct dispatch of org tools by name.
 *
...
// Re-export TeamQueryRunner for consumers that imported it from registry.ts
export type { TeamQueryRunner } from '../sessions/tools/org-tool-context.js';
    ...
    },
    {
      ...
      handler: (input, callerId, sourceChannelId) => Promise.resolve(
        delegateTask(input as never, callerId, {
          orgTree: deps.orgTree, taskQueue: deps.taskQueue, log: deps.log,
        }, sourceChannelId)
      ),
    },
    {
...
/** Create a direct tool invoker for bootstrap and E2E tests. */
export function createToolInvoker(deps: OrgToolDeps): OrgToolInvoker {
