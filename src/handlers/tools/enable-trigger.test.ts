...
describe('enable_trigger', () => {
  ...
  it('calls resetOverlapState on enable', async () => {
    const result = await server.invoke('enable_trigger', {
      team: 'ops-team', trigger_name: 'fetch-logs',
    }, 'root') as { success: boolean };

    expect(result.success).toBe(true);
    expect(mockConfigStore.resetOverlapState).toHaveBeenCalledWith('ops-team', 'fetch-logs');
  });

  it('calls resetFailures on enable', async () => {
    const result = await server.invoke('enable_trigger', {
      team: 'ops-team', trigger_name: 'fetch-logs',
    }, 'root') as { success: boolean };

    expect(result.success).toBe(true);
    expect(mockConfigStore.resetFailures).toHaveBeenCalledWith('ops-team', 'fetch-logs');
  });

  it('calls setState to active', async () => {
    const result = await server.invoke('enable_trigger', {
      team: 'ops-team', trigger_name: 'fetch-logs',
    }, 'root') as { success: boolean };

    expect(result.success).toBe(true);
    expect(mockConfigStore.setState).toHaveBeenCalledWith('ops-team', 'fetch-logs', 'active');
  });
});
