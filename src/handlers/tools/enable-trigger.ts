/**
 * enable_trigger tool — activate a trigger and register its handler.
 */
...
export const EnableTriggerInputSchema = z.object({
  team: z.string().min(1),
  trigger_name: z.string().min(1),
});

export interface EnableTriggerResult {
  readonly success: boolean;
  readonly error?: string;
}

export interface EnableTriggerDeps {
...
export function enableTrigger(
  input: z.infer<typeof EnableTriggerInputSchema>,
  callerId: string,
  deps: EnableTriggerDeps,
): EnableTriggerResult {
  ...
  deps.log('Enabled trigger', { team: input.team, trigger: input.trigger_name });
  return { success: true };
}
